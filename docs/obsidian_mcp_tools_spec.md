...
- `operon_adopt_task`: adopt one exact checkbox only when the loaded engine grants the official task-workflow preview/apply pair. Operon `3.5.3` introduced this public task-workflow boundary and the current `3.9.3` certified target retains it; Operon owns the opaque sealed plan and same-plan recovery, with no Markdown fallback.
...
the current `3.9.3` target; legacy
