...
When running in the foreground, use `Ctrl+C` to stop that client cleanly before restarting. With a service or supervisor, restart only the instance tied to the Optimike profile; do not kill every `tunnel-client` process on the machine. Read the URL from `$healthFile`, check `/healthz` and `/readyz`, then call `external_roots_list`, `external_list`, and `external_read` on a small canary file.
