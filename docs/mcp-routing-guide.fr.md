...
| Writes Markdown/frontmatter/tags/admin bornés sur copie ou vault dédié  | Optimike MCP en `headless-filesystem`                           | Sécurité de chemins, dry-run par défaut et préconditions.                         |
...
Utiliser `obsidian_validate_format` avant les writes risqués ou le contenu généré :
...
- `add_text_node` ajoute un noeud texte.
