...
Planning reads a bounded Base snapshot, checks its SHA and binding, queries with `evaluate:false`, then rereads the Base. Selection requires a complete fallback response with at most 500 rows, no warnings, unique file paths and exactly one target match. Truncation, unsupported filters, unknown view, ambiguous view names, or a changed Base/binding fail closed. The Bridge supported-filter snapshot is **not a native engine completeness guarantee**. Its metadata cache freshness is explicitly unknown.
