...
Optimike Obsidian MCP has four runtime contracts. Headless modes run over a synchronized Markdown vault. They do not run Obsidian Desktop, load community plugins, expose the command palette, or provide live UI state.
...
| `headless-guarded` | Controlled writes on a copied or dedicated vault | Not required | Not required | Guarded filesystem writes only | Local readonly fallback | Write sandbox first |

## Capability Table

| Capability | `live` | `hybrid` API available | `hybrid` API unavailable | `headless-readonly` | `headless-guarded` |
| --- | --- | --- | --- | --- | --- |
| Start without `OBSIDIAN_API_KEY` | No | Yes | Yes | Yes | Yes |
| Start without Obsidian Desktop | No | Yes | Yes | Yes | Yes |
| Filesystem cache | Optional | Yes | Yes | Required | Required |
| Vault exclusion policy | Yes for cache scans | Yes for cache scans | Yes | Yes | Yes |
| List/read/search | REST/cache | REST/cache | Cache/filesystem | Cache/filesystem | Cache/filesystem |
| Tasks list/query | Cache/filesystem | Cache/filesystem | Cache/filesystem | Cache/filesystem | Cache/filesystem |
| Smart semantic search | If `.smart-env` exists | If `.smart-env` exists | If `.smart-env` exists | If `.smart-env` exists | If `.smart-env` exists |
| Runtime status/maintenance | Yes | Yes | Yes | Yes | Yes |
| Update note | REST full tool | REST full tool | No | No | Append/prepend only |
| Search/replace | REST full tool | REST full tool | No | No | Exact filePath replacements only |
| Frontmatter | REST full tool | REST full tool | No | No | Single-key `set` only |
| Tags/delete | REST full tools | REST full tools | No | No | No |
| Active file / UI / commands | Via Desktop/plugin | Via Desktop/plugin | No | No | No |
| Bases list/schema/query | Bases Bridge REST | Bases Bridge REST | No | Local readonly fallback | Local readonly fallback |
| Bases create/upsert | Bases Bridge REST | Bases Bridge REST | No | No | No |
| Obsidian plugin parity | Desktop plugins | Desktop plugins while API available | No | No | No |
...
- `headless-guarded` should be validated on a copied or dedicated vault before any production vault write path.
