...
const Plan = z.object({
  sourcePath: z.string().min(1).max(1024).describe("Exact vault-relative Markdown source path."),
  destinationPath: z.string().min(1).max(1024).describe("Exact absent Markdown destination; its parent must already exist."),
...
export function registerNativeNoteMoveTools(server: McpServer, runtime: NativeNoteMoveOperationAdapter | undefined): void {
  if (!runtime) return;
  server.tool("obsidian_note_move_plan",
    ...
    params => run("obsidian_note_move_plan", () => runtime.plan(Plan.parse(params))));
  server.tool("obsidian_note_move_apply",
    ...
    Apply.shape, GOVERNED_MUTATION_TOOL_ANNOTATIONS,
    params => run("obsidian_note_move_apply", () => { const p = Apply.parse(params); return runtime.apply(p.planRef, p.idempotencyKey); }));
  server.tool("obsidian_note_move_status",
    ...
    params => run("obsidian_note_move_status", () => runtime.status(Status.parse(params).planRef)));
}
