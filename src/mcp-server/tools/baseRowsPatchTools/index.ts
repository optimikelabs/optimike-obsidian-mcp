...
import { z } from "zod";
import { BaseRowsPatchRuntime, type BaseRowsPatchInput } from "../../../services/baseRowsPatchRuntime.js";
...
const Operation = z.discriminatedUnion("op", [
  z.object({ op: z.literal("set"), key: z.string().min(1).max(256), value: z.unknown() }).strict(),
  z.object({ op: z.literal("delete"), key: z.string().min(1).max(256) }).strict(),
]);
const Plan = BaseRowTargetSchema.extend({
  operations: z.array(Operation).min(1).max(64).describe("Raw top-level frontmatter keys; not Base display names or file/formula properties."),
  idempotencyKey: z.string().min(1).max(256),
}).strict();
const Status = z.object({ planRef: z.string().min(1).max(128) }).strict();
const Apply = Status.extend({ idempotencyKey: z.string().min(1).max(256) }).strict();
async function run(toolName: string, operation: () => Promise<unknown>) {
  try { return { isError: false, content: [{ type: "text" as const, text: JSON.stringify(await operation()) }] }; }
  catch (error) { return { isError: true, content: [{ type: "text" as const,
    text: JSON.stringify(publicMcpToolErrorPayload(error, { operation: toolName, toolName, params: {} })) }] }; }
}
export function registerBaseRowsPatchTools(server: McpServer, runtime: BaseRowsPatchRuntime | undefined) {
  if (!runtime) return;
  server.tool("bases_rows_patch_plan",
    ...
    Plan.shape, GOVERNED_PLAN_TOOL_ANNOTATIONS,
    params => run("bases_rows_patch_plan", () => runtime.plan(Plan.parse(params) as BaseRowsPatchInput)));
  server.tool("bases_rows_patch_apply",
    ...
    Apply.shape, { ...GOVERNED_MUTATION_TOOL_ANNOTATIONS, destructiveHint: false },
    params => run("bases_rows_patch_apply", () => { const p = Apply.parse(params); return runtime.apply(p.planRef, p.idempotencyKey); }));
  server.tool("bases_rows_patch_status",
    ...
    Status.shape, READ_ONLY_TOOL_ANNOTATIONS,
    params => run("bases_rows_patch_status", () => runtime.status(Status.parse(params).planRef)));
}
