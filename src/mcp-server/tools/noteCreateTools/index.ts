...
const Plan = z.object({
  path: z.string().min(1).max(1024).describe("Exact new vault-relative Markdown path; parent directory must already exist."),
  content: z.string().max(NOTE_CREATE_MAX_BYTES).describe("Exact complete intended Markdown. Include a frontmatter envelope when qualified automatic date fields are active."),
  idempotencyKey: z.string().min(1).max(256),
}).strict();
const Apply = z.object({ planRef: z.string().min(1).max(128), idempotencyKey: z.string().min(1).max(256) }).strict();
...
export function registerNoteCreateTools(server: McpServer, runtime: NoteCreateOperationAdapter | undefined): void {
  if (!runtime) return;
  server.tool("obsidian_note_create_plan",
    ...
    params => run("obsidian_note_create_plan", () => runtime.plan(Plan.parse(params))));
  server.tool("obsidian_note_create_apply",
    ...
    params => run("obsidian_note_create_apply", () => { const p = Apply.parse(params); return runtime.apply(p.planRef, p.idempotencyKey); }));
  server.tool("obsidian_note_create_status",
    ...
    params => run("obsidian_note_create_status", () => runtime.status(Status.parse(params).planRef)));
}
