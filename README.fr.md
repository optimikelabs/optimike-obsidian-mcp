...
- **Operon Developer API V1** et **Optimike Operon Bridge 0.9.3** pour les tâches gouvernées. Optimike MCP `3.9.1` certifie Operon officiel `3.9.3` sur Developer API V1 avec Operon CLI `1.2.0` et Local REST API `5.1.0` ; l’admission de la release exige la gate Pilot 2 du dépôt sur le SHA exact. Le contrat public Developer API et la frontière task-workflow additive n’ont pas dérivé depuis `3.6.2` ; Pilot 2 vérifie preview/apply natifs, replay idempotent, refus des révisions périmées, médias typés, restauration exacte et zéro recovery en attente. Les futures releases non refusées restent `compatible-provisional` jusqu’à preuve séparée. Les trois Bridges inclus [récupèrent leurs routes Local REST après un démarrage tardif ou un rechargement](docs/bridge-lifecycle.fr.md), sans redémarrer le MCP ni modifier les autorisations d’écriture. Leur [bundle de release unique et vérifié](docs/bridge-packaging.fr.md) préserve les réglages plugin et permet un rollback clôturé.
