...
const { registerNativeNoteMoveTools } = await import(
  "../dist/mcp-server/tools/nativeNoteMoveTools/index.js"
);
const { NativeNoteMoveOperationAdapter } = await import(
...
const { sealNativeMove, nativeDigest } = await import(
  "../dist/services/nativeNoteMoveContract.js"
);
...
let calls = 0;
const preflight = sealNativeMove({
  sourcePath: "A.md",
...
const backend = {
  ...
  async apply(r) {
    ...
    const result = {
      ...
      preconditionDigest: r.preconditionDigest,
      bindingFingerprint: r.bindingFingerprint,
      outcome: "committed",
      reason: "native_rename_returned",
      afterSha256: nativeDigest("source"),
      graphPostflight: "pending",
      scope: "sealed_neighborhood_only",
      replayAllowed: false,
    };
    replies.set(r.operationId, result);
    return result;
  },
  async status(id, digest) {
    return (
      replies.get(id) ?? {
        contractVersion: 1,
        operationId: id,
        ...
        reason: "backend_receipt_unavailable",
...
try {
  ...
  const runtime = new NativeNoteMoveOperationAdapter(
    backend,
    journal,
    () => {},
  );
  ...
  assert.deepEqual(
    selectAvailableToolProfileNames({
      profile: "standard",
      availableNames: names.slice(0, 2),
    }),
    [],
  );
  assert.deepEqual(
  ...
  const planned = parse(
    await call("obsidian_note_move_plan", {
      sourcePath: "A.md",
      destinationPath: "B.md",
      idempotencyKey: "once",
    }),
  ...
  assert.equal(applied.graph_postflight, "pending");
  await call("obsidian_note_move_apply", {
    planRef: planned.planRef,
    idempotencyKey: "once",
  });
  ...
  const invalid = await call("obsidian_note_move_status", {
    planRef: "obsidian-note-replace:v1:00000000-0000-4000-8000-000000000000",
  });
  assert.equal(invalid.isError, true);
  assert.doesNotMatch(
    JSON.stringify(invalid),
    /plans\.sqlite|bindingFingerprint|nextContent/,
  );
  const uncertain = await runtime.plan({
    sourcePath: "A.md",
    destinationPath: "B.md",
  ...
  const cockpit = new OperationCockpit([
    {
      listPendingOperationRows(input) {
        return journal.listPendingOperationRows({
          ...
          admittedProjectionKinds: ["obsidian.note.move"],
          allowUnprojectedFallback: false,
        });
      },
    },
  ]);
  const pending = cockpit
    .list()
    .operations.find((o) => o.planRef === uncertain.planRef);
  assert.equal(pending.operationKind, "obsidian.note.move");
  assert.equal(pending.nextAction, "status");
  assert.equal(calls, 1);
  console.log(
    "PASS: M3 real MCP protocol, annotations, readonly policy, complete three-tool profile, domain fence and cockpit status-only recovery",
  );
} finally {
  await client.close();
  await server.close();
