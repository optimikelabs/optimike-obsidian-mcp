...
import {
  ...
  FRONTMATTER_FIXTURE_PATH as notePath,
...
const fixture = new GovernedFrontmatterAtomicServer();
...
const { createGovernedNoteReplaceRuntime } = await import(
  "../dist/mcp-server/tools/governedNoteReplaceTools/runtime.js"
);
const { BaseRowSelectionReader } = await import(
  "../dist/services/baseRowSelection.js"
);
const { BaseRowsPatchRuntime, BASE_ROWS_PREFIX } = await import(
  "../dist/services/baseRowsPatchRuntime.js"
);
const { registerBaseRowsPatchTools } = await import(
...
const input = {
  ...
  path: notePath,
  operations: [
    { op: "set", key: "statut", value: "closed" },
    { op: "delete", key: "owner" },
  ],
  idempotencyKey: "once",
};
    ...
    async query() {
      ...
      return {
        ...
        rows: present
          ? [
              {
                file: { path: notePath, name: "Governed Frontmatter" },
                props: {},
              },
            ]
          : [],
...
try {
  {
    ...
    const childRef = p.planRef.replace(
      BASE_ROWS_PREFIX,
    ...
    assert.equal(result.recoveryRef, undefined);
    assert.equal(result.idempotencyKey, undefined);
    assert.equal(fixture.successfulWrites, writes + 1);
    assert.equal(
      fixture.content.split("---\n").at(-1),
      FRONTMATTER_INITIAL_CONTENT.split("---\n").at(-1),
    );
    assert.match(fixture.content, /statut: "closed"/);
    assert.doesNotMatch(fixture.content, /owner:/);
    const reads = f.queries();
    f.remove();
    assert.equal((await f.runtime.status(p.planRef)).outcome, "committed");
    assert.equal(
      (await f.runtime.apply(p.planRef, input.idempotencyKey)).outcome,
      "committed",
    );
    assert.equal(
      f.queries(),
      reads,
      "postpatch view membership is not a new mutation prerequisite",
    );
    assert.equal(fixture.successfulWrites, writes + 1);
    assert.doesNotMatch(
      JSON.stringify(result),
      /Private prose|nextContent|projection:v1:/,
    );
    f.notes.close();
    cases++;
  }
  for (const change of ["changeBase", "rebind", "remove"]) {
    const f = scenario(change),
      p = await f.runtime.plan(input),
      writes = fixture.successfulWrites;
    f[change]();
    await assert.rejects(f.runtime.apply(p.planRef, input.idempotencyKey));
    assert.equal(fixture.successfulWrites, writes);
    f.notes.close();
    cases++;
  }
  {
    const f = scenario("source-conflict"),
      p = await f.runtime.plan(input),
      writes = fixture.successfulWrites;
  ...
  {
    const f = scenario("readonly"),
      p = await f.runtime.plan(input),
      writes = fixture.successfulWrites;
    const old = config.mcpWriteMode;
    config.mcpWriteMode = "readonly";
    try {
      await assert.rejects(f.runtime.apply(p.planRef, input.idempotencyKey));
      await f.runtime.status(p.planRef);
    } finally {
      config.mcpWriteMode = old;
    }
    assert.equal(fixture.successfulWrites, writes);
    f.notes.close();
    cases++;
  ...
  for (const key of [
    ...
    const f = scenario("key-" + cases),
      writes = fixture.successfulWrites;
    await assert.rejects(
      f.runtime.plan({
        ...input,
        operations: [{ op: "set", key, value: "unsafe" }],
      }),
    );
    assert.equal(fixture.successfulWrites, writes);
    f.notes.close();
  ...
  for (const after of [false, true]) {
    const f = scenario("loss-" + after),
      p = await f.runtime.plan(input),
      calls = fixture.casRequests;
    if (after) fixture.loseResponseAfterWriteNext = true;
    else fixture.failBeforeWriteNext = true;
    ...
    assert.equal(
      fixture.casRequests,
      calls + 1,
      "unknown child may be observed, never blindly reapplied",
    );
    restarted.notes.close();
    cases++;
  }
  {
    ...
    assert.notEqual(duplicate.outcome, "committed");
    gate.release();
  ...
  }
  {
    ...
    assert.equal(
      ...
      writes + 1,
      "the stale caller must not redispatch",
    );
    second.close();
    f.notes.close();
    cases++;
  ...
  {
    const f = scenario("protocol"),
      server = new McpServer({ name: "rows-fixture", version: "1" }),
      client = new Client({ name: "test", version: "1" });
    const [ct, st] = InMemoryTransport.createLinkedPair();
    registerBaseRowsPatchTools(server, f.runtime);
    ...
    try {
      ...
      const result = await client.callTool({
        name: "bases_rows_patch_apply",
        arguments: { planRef: p.planRef, idempotencyKey: input.idempotencyKey },
      });
      assert.equal(JSON.parse(result.content[0].text).outcome, "committed");
      const invalid = await client.callTool({
        name: "bases_rows_patch_status",
        arguments: { planRef: "bad" },
      });
      assert.equal(invalid.isError, true);
      assert.doesNotMatch(JSON.stringify(invalid), /\.sqlite|nextContent/);
    } finally {
      await client.close();
      await server.close();
      f.notes.close();
