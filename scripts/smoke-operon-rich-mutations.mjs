...
const inlineTargetPath =
  process.env.OPERON_MUTATION_INLINE_TARGET ?? "Pilot.md";
...
function expectStatus(result, status, label) {
  if (result.status !== status)
    throw new Error(
      `${label}: expected ${status}, got ${JSON.stringify(result)}`,
    );
  return result;
}
...
async function main() {
  ...
  const client = new Client({
    name: "operon-rich-mutation-smoke",
    version: "1",
  });
  try {
    await client.connect(transport);
    const blocker = expectStatus(
      await call(client, "operon_create_task", {
    ...
    if (blocked.after?.checkbox !== "open")
      throw new Error("Blocked child changed despite rejection.");
    ...
    const blockerDone = expectStatus(
      await call(client, "operon_transition_task", {
        operonId: blockerId,
    ...
    const childDone = expectStatus(
      ...
      "applied",
      "child transition after blocker",
    );

    const convertedFile = expectStatus(
      await call(client, "operon_convert_task", {
        operonId: childId,
    ...
    if (convertedFile.after?.source !== "file")
      throw new Error("Inline-to-file conversion did not change source.");

    const convertedInline = expectStatus(
      await call(client, "operon_convert_task", {
        operonId: childId,
      ...
      "applied",
      "file to inline conversion",
    );
    if (
      convertedInline.after?.source !== "inline" ||
      convertedInline.after?.path !== inlineTargetPath
    ) {
      throw new Error(
        "File-to-inline conversion did not reach the requested target.",
      );
    }
