...
// DORMANT CONTRACT — this archive is not evidence for the current fail-closed runtime.
// Reactivate it only after Optimike ships an audited native handle-relative mutation
// primitive and explicitly opts into this historical mutation/recovery contract.
const NATIVE_CONTRACT_TEST_CONFIRMATION = "I_HAVE_AN_AUDITED_NATIVE_PRIMITIVE";
if (
  process.env.MCP_EXTERNAL_MOVE_NATIVE_CONTRACT_TEST !==
  NATIVE_CONTRACT_TEST_CONFIRMATION
) {
  console.log(
    "SKIP external_move native mutation contract: audited handle-relative primitive unavailable.",
  );
  process.exit(0);
}
